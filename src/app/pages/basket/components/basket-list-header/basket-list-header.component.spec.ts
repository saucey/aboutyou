...
describe('BasketListHeaderComponent', () => {
  ...
  describe('.header .info .label', () => {
    it('should render app-text component', () => {
      const textComponentDebugElement = debugElement.query(By.directive(TextComponent));
      expect(textComponentDebugElement).toBeDefined();
    });

    it('should render label', () => {
      const textComponentDebugElement = debugElement.query(By.directive(TextComponent));
      const label = (textComponentDebugElement.componentInstance as TextComponent).label;
      expect(label).toContain('BASKET.listHeader.label');
    });

    it('should not render label if showTitle = false', () => {
